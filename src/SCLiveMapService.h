...
/*!
 @breif LiveMapServer로부터 연결이 실패하였을 때 호출됨.
...
/*!
 @breif LiveMapServer로부터 통신도중 연결이 끊어졌을 때 호출됨.
...
/*!
 @breif LiveMapService 객체를 사용할 준비가 되었을때 호출됨.
...
/*!
 @breif 사용자의 주변 위치에 클라이언트가 존재할 떄 호출됨.
...
/*!
 @breif 사용자가 다른 클라이언트로부터 메시지를 수신받았을 때 호출됨.
...
-(void)asyncStart;
/*!
 @breif 라이브맵 서비스 종료 매서드.
 @details LiveMapServer와 연결을 종료하고 서비스를 종료합니다.
 */
-(void)stop;
/*!
 @breif 라이브맵 사용자의 위치를 업데이트 한다.
...
/*!
 @breif 라이브맵 사용자의 위치와 사용자의  시점를 업데이트 한다.
 @details LiveMapServer가 사용자의 위치를 이 메서드로 받고 실시간으로 업데이트 한다.
...
/*!
 @breif LiveMapServer에 연결되어있는 다른 클라이언트에게 메시지 전송
...
/*!
 @breif SCLiveMapServiceDelegate 프로퍼티
 */
